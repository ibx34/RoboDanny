...
class Stats(commands.Cog):
    ...
    async def about(self, ctx):
        ...
        embed = discord.Embed(
            description="**This is a self hosted version of R. Danny**\nLatest Changes On [GitHub](https://github.com/Rapptz/RoboDanny)\n"
        )
        embed.title = "R. Danny Support Server"
        embed.url = "https://discord.gg/DWEaqMy"
        embed.colour = discord.Colour.blurple()

        # To properly cache myself, I need to use the bot support server.
        support_guild = self.bot.get_guild(825512897080852520)
        owner = await self.bot.get_or_fetch_member(support_guild, 366649052357591044)
    ...
    async def show_guild_stats(self, ctx):
        ...
        embed = discord.Embed(
            title="Server Command Stats", colour=discord.Colour.blurple()
        )
    ...
    async def stats_today(self, ctx):
        ...
        e = discord.Embed(
            title="Last 24 Hour Command Stats", colour=discord.Colour.blurple()
        )
        e.description = (
    ...
    async def notify_gateway_status(self, record):
        ...
        msg = textwrap.shorten(
            f"{emoji} `[{dt:%Y-%m-%d %H:%M:%S}] {record.message}`", width=1990
        )
